...
SERIES = {"PCEPI": "headline", "PCEPILFE": "core"}
...
def main():
    ...
    con.execute("DROP TABLE IF EXISTS raw.raw_fred__pce;")
    con.execute("""
        CREATE TABLE raw.raw_fred__pce (
            series_id                   VARCHAR,
    ...
    """)
    con.executemany(
        f"INSERT INTO raw.raw_fred__pce VALUES ({','.join(['?'] * 25)})",
        [tuple(r.values()) for r in pce_rows],
    )

    n_meta = load_series_meta_table(con, "raw.raw_fred__pce_series_meta", meta_rows)

    n_pce = con.execute("SELECT count(*) FROM raw.raw_fred__pce").fetchone()[0]
    con.close()
    print(f"Loaded {n_pce} raw rows into raw.raw_fred__pce")
    print(f"Loaded {n_meta} raw rows into raw.raw_fred__pce_series_meta")
